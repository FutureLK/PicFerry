...
| 单文件打包 | PyInstaller → 约 6.7 MB，零依赖 |
...
- **悬浮大图预览** — 鼠标悬停文件名后弹出大图预览面板，自动避让视口边缘（延迟可在「更多设置」调节）
...
4. 查重为**分p级匹配**：本地 `123_p2.jpg` 仅当书签中作品 123 的页数 > 2 才判定为已收藏；结果表显示每个文件的命中分p（如 `p2/共4p`）
...
├── archived/          旧版 Node.js 项目（仅本地保留，不入库）
└── README.md
```

## 历史版本

- **Node.js 旧版**（约 38MB，不再维护）仅保存在本地 `archived/` 目录，不入库、不推送。
