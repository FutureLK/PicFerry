...
| 1815+ | 本地路径净化：`_safe_error_text`(1817)/`_sanitize_rel_path`(1824)/`_declared_local_bases`(1840)/`_check_local_base`(1850)/`_check_realpath_within`(1859)，防目录穿越；扫描入口 `_assert_declared_scan_base`(1867) 与读链路同口径 | 路径安全相关 |
| 2006+ | `fetch_all_pixiv_bookmark_ids`：Pixiv 收藏拉取（分页/黑名单/限量/可终止） | Pixiv 相关逻辑 |
| 2122-2258 | **Pixiv 后台 Job 引擎（单槽）**：`pixiv_job` 状态字典(2122) → `run_pixiv_job`(2148) 状态机 `fetching→scanning→matching→done/stopped/error` → `_start_pixiv_job`(2243) 原子启动。关键时序：**每个昂贵阶段前先检查 `stop`** | 新的后台任务引擎放这类区域 |
| 2261-2656 | **HTTP 层** `SyncHandler(BaseHTTPRequestHandler)`：`_send_json`(2267)/`_send_html`(2276)/`_send_error`(2284)/`_check_origin`(2292)/`do_OPTIONS`(2317)/`do_GET`(2325)/`do_POST`(2354) + 各 `_handle_*` 处理器 | 新 API handler 加在这里 |
| 2658+ | `ThreadedServer`：多线程 HTTP 服务器 + `__main__` 启动（自动开浏览器） | 启动逻辑 |

## 2. 关键约定

- 新增 API 端点：在 `do_GET`(2325) 或 `do_POST`(2354) 加 elif 分支 → 写 `_handle_*` 方法 → 更新 `docs/api.md`。
- 新增配置：在 `_CONFIG_KEYS`(78) 注册（若需范围/类型）→ `load_config`/`save_config` 自动覆盖（数值键）→ 前端表单同步 → 更新 `docs/settings.md`。
- 请求体为 JSON 时：读 `Content-Length` → `self.rfile.read` → `json.loads`，参考 `_handle_pixiv_bookmarks`(2459)。
...
需要后台任务时，**复制而不是重写**现有 Pixiv Job 引擎模式（`server.py:2122-2258`）：
- 状态字典 + `stop` 事件 + `lock`（参考 `pixiv_job`，`server.py:2122`）。
...
| 1815+ | Local-path sanitization: `_safe_error_text`(1817)/`_sanitize_rel_path`(1824)/`_declared_local_bases`(1840)/`_check_local_base`(1850)/`_check_realpath_within`(1859), traversal guard | Path-safety code |
| 2006+ | `fetch_all_pixiv_bookmark_ids`: Pixiv bookmark fetching (paging/blacklist/limit/stoppable) | Pixiv logic |
| 2122-2258 | **Pixiv Job engine (single-slot)**: `pixiv_job` state dict(2122) → `run_pixiv_job`(2148) state machine `fetching→scanning→matching→done/stopped/error` → `_start_pixiv_job`(2243) atomic start. Key timing: **check `stop` before every expensive phase** | New background-task engines |
| 2261-2656 | **HTTP layer** `SyncHandler(BaseHTTPRequestHandler)`: `_send_json`(2267)/`_send_html`(2276)/`_send_error`(2284)/`_check_origin`(2292)/`do_OPTIONS`(2317)/`do_GET`(2325)/`do_POST`(2354) + `_handle_*` handlers | New API handlers |
| 2658+ | `ThreadedServer`: threaded HTTP server + `__main__` startup (auto-opens browser) | Startup logic |

## 2. Key conventions

- New API endpoint: add elif branch in `do_GET`(2325) or `do_POST`(2354) → write `_handle_*` method → update `docs/api.md`.
...
- JSON request bodies: read `Content-Length` → `self.rfile.read` → `json.loads`, see `_handle_pixiv_bookmarks`(2459).
...
- Handlers use `_send_json`/`_send_html`/`_send_error` (2267/2276/2284) — never hand-roll `send_response`.
...
**Copy, don't rewrite**, the Pixiv Job engine pattern (`server.py:2122-2258`): state dict + `stop` event + `lock` (2122); check `stop` before every expensive phase (order matters, see `run_pixiv_job` comments); atomic check-and-set start (2243) rejecting concurrent runs; split status vs result endpoints.
