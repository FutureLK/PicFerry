...
- **响应辅助**：HTTP handler 一律用 `_send_json`/`_send_html`/`_send_error`（`server.py:2133`/`2142`/`2150`），不要手写 `send_response` 序列。
...
- Handlers use `_send_json`/`_send_html`/`_send_error` (2133/2142/2150) — never hand-roll `send_response`.
