...
`server.py`（约 57 行）是服务端装配与启动入口：精简导入区（含 debug 导入计时）+ `PORT` + `ThreadedServer` + `main()`（信号处理）；HTTP 路由层在 `handler.py`（`SyncHandler`，约 514 行，Pixiv API 接口文档注释在类定义上方）。前端 HTML/CSS/JS 已拆分为 `static/` 真实文件（index.html / style.css / app.js），由 `webassets.py` 在导入期装配回完整 `HTML`；日志/配置/路径安全/数据源/Pixiv 引擎外移到同目录一方模块（logging_util.py / config_store.py / pathsafety.py / datasources.py / pixiv.py / handler.py）。本文档是**代码内部结构地图** + 模块组织约定，说明每个行区放什么、新增代码该放哪里、按什么风格写。
...
| 行区 | 内容 | 新增代码放哪 |
|---|---|---|
| 1-21 | 导入：纯标准库(1-7) + 一方模块计时导入（logging_util / config_store / handler），导入期 `perf_counter` 计时写入 `logging_util.IMPORT_TIMING`（debug 诊断数据源；**pathsafety / datasources / webassets / pixiv 四模块由 handler.py 导入区计时**） | 新 import 加到这里（**仅标准库 + 一方模块**） |
| 23 | `PORT=13826`（`IMAGE_EXTS` 已外移 datasources.py） | 模块级常量加在附近 |
| 26-57 | `ThreadedServer`(28)：多线程 HTTP 服务器 + `main`(32)/`__main__` 启动（SIGINT/SIGTERM 信号处理、自动开浏览器） | 启动逻辑 |
...
| — | **handler.py**：`SyncHandler(BaseHTTPRequestHandler)` HTTP 路由层（`_check_origin` / `do_GET` / `do_POST` + 全部 `_handle_*`）+ `strip_remote_locked_keys`（声明权剥除）+ `_emit_import_timing`（debugMode 关→开时补发模块导入耗时，数据只读自 `logging_util.IMPORT_TIMING`）；导入区对 pathsafety / datasources / webassets / pixiv 四模块 `perf_counter` 计时（迁自 server.py，计时对与 import 行是绑定单元）；类上方为 Pixiv API 接口文档注释 | 新 API handler 加在这里 |

## 2. 关键约定

- 新增 API 端点：在 `do_GET`(handler.py:175) 或 `do_POST`(handler.py:204) 加 elif 分支 → 写 `_handle_*` 方法 → 更新 `docs/api.md`。
...
- 请求体为 JSON 时：读 `Content-Length` → `self.rfile.read` → `json.loads`，参考 `_handle_pixiv_bookmarks`(handler.py:309)。
- **导入计时对是绑定单元**：一方模块 from-import 与其前后的 `_t_imp` 赋值、`IMPORT_TIMING` 写入必须同进同出（server.py 与 handler.py 导入区均如此），严禁只删 import 行留下孤儿计时对（会产生 ≈0ms 假数据）。
- 所有 API 响应 JSON 用 `ensure_ascii=False`，中文直接输出。

## 3. 风格约定

- **中文注释 + 分区分隔线**：逻辑块之间用 `# ─── 分区名 ───` 分隔（如 `handler.py:30` 的 `# ─── 声明权控制 ───`）。
...
- **响应辅助**：HTTP handler 一律用 `_send_json`/`_send_html`/`_send_error`（`handler.py:117`/`126`/`134`），不要手写 `send_response` 序列。

## 4. 前后端边界

- 前端已拆分为真实文件：`static/index.html`（骨架）+ `static/style.css` + `static/app.js`，由 **webassets.py** 在导入期替换占位符装配成完整 `HTML`（`handler.py:23` 导入）。**没有内嵌 HTML 字符串**；改 UI = 改 `static/` 对应文件，改装配逻辑 = 改 webassets.py。
- 前后端交互只走 `/api/*` 端点（见 `docs/api.md`）；同源页面受 `_check_origin` 保护，无跨域问题。
- 前端给后端传参：GET 用 query string，POST 用 JSON body（`Content-Length` + `rfile.read` + `json.loads`，见 `_handle_pixiv_bookmarks`，`handler.py:309`）。
...
- 每次改动后：`python -m py_compile server.py handler.py logging_util.py config_store.py pathsafety.py datasources.py pixiv.py webassets.py verify.py`（必做）。
...
- [ ] `python -m py_compile server.py handler.py logging_util.py config_store.py pathsafety.py datasources.py pixiv.py webassets.py verify.py` 通过
...
`server.py` (~57 lines) is the assembly & startup entry: trimmed import block (with debug import timing) + `PORT` + `ThreadedServer` + `main()` (signal handling). The HTTP routing layer lives in `handler.py` (`SyncHandler`, ~514 lines; the Pixiv API doc comment sits above the class). The frontend HTML/CSS/JS is split into real files under `static/` (index.html / style.css / app.js), assembled back into the full `HTML` string by `webassets.py` at import time. Logging/config/path-safety/datasources/Pixiv engine live in sibling first-party modules (logging_util.py / config_store.py / pathsafety.py / datasources.py / pixiv.py / handler.py). This document is the **internal structure map** + module conventions: what each region holds, where new code goes, and how to write it.
...
|---|---|---|
| 1-21 | Imports: stdlib(1-7) + timed first-party imports (logging_util / config_store / handler), each wrapped in `perf_counter` timing written to `logging_util.IMPORT_TIMING` (debug diagnostics; **pathsafety / datasources / webassets / pixiv are timed in handler.py's import block**) | New imports go here (stdlib + first-party only) |
| 23 | `PORT=13826` (`IMAGE_EXTS` moved to datasources.py) | Module-level constants |
| 26-57 | `ThreadedServer`(28): threaded HTTP server + `main`(32)/`__main__` startup (SIGINT/SIGTERM handling, auto-opens browser) | Startup logic |
...
| — | **handler.py**: `SyncHandler(BaseHTTPRequestHandler)` HTTP routing layer (`_check_origin` / `do_GET` / `do_POST` + all `_handle_*`) + `strip_remote_locked_keys` (declaration guard) + `_emit_import_timing` (re-emits module import timing when debugMode goes off→on, read-only from `logging_util.IMPORT_TIMING`); its import block times pathsafety / datasources / webassets / pixiv via `perf_counter` (moved from server.py — timing pairs and import lines are bound units); the Pixiv API doc comment sits above the class | New API handlers |

## 2. Key conventions

- New API endpoint: add elif branch in `do_GET`(handler.py:175) or `do_POST`(handler.py:204) → write `_handle_*` method → update `docs/api.md`.
...
- JSON request bodies: read `Content-Length` → `self.rfile.read` → `json.loads`, see `_handle_pixiv_bookmarks`(handler.py:309).
- Import-timing pairs are bound units: each first-party from-import moves/deletes together with its surrounding `_t_imp` assignment and `IMPORT_TIMING` write (in both server.py and handler.py import blocks) — never leave an orphan timing pair behind a deleted import (it would record a fake ≈0 ms entry).
- All API JSON responses use `ensure_ascii=False`; Chinese output directly.

## 3. Style

- Chinese comments + `# ─── section ───` separators between logic blocks (e.g. `handler.py:30`).
...
- Handlers use `_send_json`/`_send_html`/`_send_error` (handler.py:117/126/134) — never hand-roll `send_response`.

## 4. Frontend/backend boundary

- The frontend is split into real files: `static/index.html` (skeleton) + `static/style.css` + `static/app.js`, assembled into the full `HTML` by **webassets.py** at import time (imported at `handler.py:23`). There is **no embedded HTML string**; UI changes go to the `static/` files, assembly changes to webassets.py.
- Frontend ↔ backend only via `/api/*` (see `docs/api.md`); same-origin pages are protected by `_check_origin`.
- Params: query string for GET, JSON body for POST (`Content-Length` + `rfile.read` + `json.loads`, see `_handle_pixiv_bookmarks`, handler.py:309).
...
- After every change: `python -m py_compile server.py handler.py logging_util.py config_store.py pathsafety.py datasources.py pixiv.py webassets.py verify.py` (mandatory).
...
- [ ] `python -m py_compile server.py handler.py logging_util.py config_store.py pathsafety.py datasources.py pixiv.py webassets.py verify.py` passes
