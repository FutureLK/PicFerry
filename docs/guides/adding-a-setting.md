...
```
_CONFIG_KEYS (config_store.py:60)  ← 注册默认值/范围/类型
   │
   ├─→ _parse_config_value (71)  自动处理解析/钳制（数值键）
   ├─→ load_config (122)          自动读取
   ├─→ save_config (131)          自动保存
...
**步骤 1：注册键。** 在 `_CONFIG_KEYS`（`config_store.py:60`）加一行：
...
注册后 `_parse_config_value`(71) 自动处理：空串/非数字回落默认、越界钳制、bool 转 `0/1`。

**步骤 2：验证自动覆盖。** `load_config`(122) 会遍历 `_CONFIG_KEYS` 读取并解析；`save_config`(131) 会遍历注册表保存（bool 键强制转 `0/1`）。`_handle_config`（GET `/api/config`）与 `_handle_config_save`（POST `/api/config/save`）同样自动覆盖。**无需改这三处。**
...
1. `load_config`（`config_store.py:122`）：在初始 dict 中加 `'MyFolder': cfg.get('Settings', 'MyFolder', fallback='')`。
2. `save_config`（`config_store.py:131`）：把键加进 `keys` 列表（config_store.py:141），并按字符串分支处理。
...
- [ ] `python -m py_compile server.py config_store.py handler.py` 通过
...
```
_CONFIG_KEYS (config_store.py:60)   ← register default/range/type
   ├─→ _parse_config_value (71)  auto parse/clamp (numeric keys)
   ├─→ load_config (122)          auto read
   ├─→ save_config (131)          auto save
...
1. Register in `_CONFIG_KEYS` (`config_store.py:60`): `'sampleSize': (64, 8, 256, 'int'),` — format `key: (default, lo, hi, type)`, `type ∈ {'int','float','bool'}`.
2. Auto-coverage: `load_config`(122) / `save_config`(131) / `_handle_config` / `_handle_config_save` all iterate the registry. No manual edits there.
...
Only when clamping is unnecessary. Four edits: `load_config`(122) initial dict, `save_config`(131) `keys` list (config_store.py:141), `_handle_config` GET echo dict, frontend form + docs.
...
- [ ] `python -m py_compile server.py config_store.py handler.py` passes
