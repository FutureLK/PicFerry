...
6. **代码引用格式**：`path:line + 符号名`（如 `config_store.py:60 _CONFIG_KEYS`）。行号为编写时快照，代码演进后可能漂移；定位源码请以符号名为准。
...
6. **Code references**: `path:line + symbol` format (e.g. `config_store.py:60 _CONFIG_KEYS`). Line numbers are a snapshot — they drift as code evolves; locate source by symbol name.
