...
- **更多设置** — 界面主题（深色/浅色日间模式）、缩略图尺寸、预览悬浮延迟、Pixiv 请求间隔、结果行数上限；作品黑名单管理
- **日志** — 实时彩色命令行日志（自动滚动 / 暂停 / 清空）

### 设备同步

- **双向查重** — B→A / A→B 一键切换，重新计算去重结果，同步方向自动跟随
- **缩略图预览** — 缩略图通过 `/api/image` 代理懒加载（IntersectionObserver 按需加载），尺寸可在「更多设置」下拉选择档位（特小16/小24/标准48/大64/特大96 px）
- **日间模式** — 「更多设置 → 界面主题」下拉切换深色/浅色（浅色为 GitHub Light 风格），选择自动保存，下次打开保持
...
- `PixivL` — 查重用的本地文件夹路径
- `thumbnailSize` — 缩略图尺寸 px（16-128，默认 48；界面为下拉档位 16/24/48/64/96，档位外旧值自动显示为"自定义"）
